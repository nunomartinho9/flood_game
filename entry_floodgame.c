...
int entry(int argc, char **argv) {
	...
	window.x = 200;
	window.y = 90;
	window.clear_color = hex_to_rgba(0xa8bf56ff);
...
/* 		float aspect = (f32)window.width/(f32)window.height;
		...
		player_xform         = m4_translate(player_xform, v3(-.25f, -.25f, 0));
		draw_image_xform(player_image, player_xform, v2(.5f, .5f), COLOR_WHITE);
